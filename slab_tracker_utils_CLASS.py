...
def make_grid_interpolator(grdfile, interp='Spherical'):
...
def getSubductionBoundarySections(shared_boundary_sections):
...
def warp_subduction_segment(tessellated_line,
    ...
    for warped_time in np.arange(time, warped_end_time-time_step,-time_step):
        ...
        for point_index, point in enumerate(points):
            ...
            if len(dip_data) > 1:
                dip_index = get_dip_angle_from_slab2(point_lat, point_lon, dip_data[1])
                #print(dip_index)
                #index returns [distance][index of array]
                dip_angle_degrees = dip_data[0].values[dip_index]

                dip_angle_radians = np.radians(dip_angle_degrees)
            else:
                dip_angle_radians(np.radians(dip_data))
