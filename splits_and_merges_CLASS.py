...
def get_topologies_and_plate_id_list(reconstruction_model,time):

    tmp_plate_snapshot = reconstruction_model.plate_snapshot(time)

    plate_id_list = tmp_plate_snapshot.plate_ids

    plate_area_list = tmp_plate_snapshot.plate_areas

    return tmp_plate_snapshot.resolved_topologies,plate_id_list,plate_area_list


def find_plate_change_for_time_step(reconstruction_model,time_from,time_to,verbose=True):

    (resolved_topologies_t1,
     plate_id_list_t1,
     plate_area_list_t1) = get_topologies_and_plate_id_list(reconstruction_model,
                                                            time_to)
    (resolved_topologies_t2,
     plate_id_list_t2,
     plate_area_list_t2) = get_topologies_and_plate_id_list(reconstruction_model,
                                                            time_from)
...
def get_plate_disappearance_time_lut(reconstruction_model,
                                     time_list,
                                     verbose=False):
    ...
    for time in time_list:
        ...
        time_to = time
        (appearing_plate_map,
         disappearing_plate_map) = find_plate_change_for_time_step(reconstruction_model,
                                         time_from,time_to,verbose)
